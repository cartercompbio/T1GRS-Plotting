...
# Remove rows with NA in predictors or outcome, as they can't be used
ALL_full <- ALL_full[complete.cases(ALL_full$DISEASE, ALL_full$T1GRS_ALL, ALL_full$Total_sum), ]

# PR Curves for Full Model
# Scores for positive class (DISEASE = 1), Scores for negative class (DISEASE = 0)
pr_prob_full <- pr.curve(scores.class0 = ALL_full$T1GRS_ALL[ALL_full$DISEASE == 2],
                         scores.class1 = ALL_full$T1GRS_ALL[ALL_full$DISEASE == 1],
                         curve = TRUE)
auprc_prob_full <- pr_prob_full$auc.integral

pr_grs2_full <- pr.curve(scores.class0 = ALL_full$Total_sum[ALL_full$DISEASE == 2],
                         scores.class1 = ALL_full$Total_sum[ALL_full$DISEASE == 1],
                         curve = TRUE)
auprc_grs2_full <- pr_grs2_full$auc.integral

print(paste("Full Model T1GRS AUPRC:", round(auprc_prob_full, 3)))
print(paste("Full Model GRS2 AUPRC:", round(auprc_grs2_full, 3)))

pr_prob_df_full <- data.frame(Recall = pr_prob_full$curve[,1], Precision = pr_prob_full$curve[,2], Model = "T1GRS")
pr_grs2_df_full <- data.frame(Recall = pr_grs2_full$curve[,1], Precision = pr_grs2_full$curve[,2], Model = "GRS2")
pr_df_full <- bind_rows(pr_prob_df_full, pr_grs2_df_full)

# Baseline for PR curve (prevalence)
baseline_pr_full <- sum(ALL_full$DISEASE == 1) / nrow(ALL_full)

# Plot PR for Full Model
plot_pr_full <- ggplot(pr_df_full, aes(x = Recall, y = Precision, color = Model)) +
  geom_path(linewidth = 0.8) +
  geom_hline(yintercept = baseline_pr_full, linetype="dashed", color="grey50") + # Baseline
  theme_minimal(base_size = 16) + theme(legend.title = element_blank(),
                                        panel.grid.major = element_blank(),
                                        panel.grid.minor = element_blank(),
                                        axis.line.x = element_line(color = "black", linewidth = 0.5),
                                        axis.line.y = element_line(color = "black", linewidth = 0.5)) +
  labs(title="NPOD: Full Model\nAUPRC Curve", x = "Recall", y = "Precision", color = "Model") +
  ylim(0, 1) + # Precision is between 0 and 1
  annotate("text", x = 0.65, y = 0.15, label = paste0("T1GRS AUPRC = ", round(auprc_prob_full, 3)), color = "slateblue", size = 5) +
  annotate("text", x = 0.65, y = 0.1, label = paste0("GRS2 AUPRC = ", round(auprc_grs2_full, 3)), color = "firebrick", size = 5) +
  annotate("text", x = 0.65, y = 0.05, label = paste0("Baseline = ", round(baseline_pr_full, 3)), color = "grey50", size = 5) +
  scale_color_manual(values = c("T1GRS" = "slateblue", "GRS2" = "firebrick"))
print(plot_pr_full)
#ggsave('figs_may/fig3/NPOD_auprc_full.pdf', plot = plot_pr_full, width = 6, height = 5)
...
# Remove rows with NA in predictors or outcome, as they can't be used
ALL_full <- ALL_full[complete.cases(ALL_full$DISEASE, ALL_full$T1GRS_MHC, ALL_full$Total_sum_HLA), ]

# PR Curves for Full Model
# Scores for positive class (DISEASE = 1), Scores for negative class (DISEASE = 0)
pr_prob_full <- pr.curve(scores.class0 = ALL_full$T1GRS_MHC[ALL_full$DISEASE == 2],
                         scores.class1 = ALL_full$T1GRS_MHC[ALL_full$DISEASE == 1],
                         curve = TRUE)
auprc_prob_full <- pr_prob_full$auc.integral

pr_grs2_full <- pr.curve(scores.class0 = ALL_full$Total_sum_HLA[ALL_full$DISEASE == 2],
                         scores.class1 = ALL_full$Total_sum_HLA[ALL_full$DISEASE == 1],
                         curve = TRUE)
auprc_grs2_full <- pr_grs2_full$auc.integral
...
plot_pr_full <- ggplot(pr_df_full, aes(x = Recall, y = Precision, color = Model)) +
  ...
  scale_color_manual(values = c("T1GRS" = "slateblue", "GRS2" = "firebrick"))
print(plot_pr_full)
#ggsave('figs_may/fig3/NPOD_auprc_MHCOnly.pdf', plot = plot_pr_full, width = 6, height = 5)
...
# Remove rows with NA in predictors or outcome, as they can't be used
ALL_full <- ALL_full[complete.cases(ALL_full$DISEASE, ALL_full$T1GRS_nonMHC, ALL_full$sum_All5_nonMHC), ]

# PR Curves for Full Model
# Scores for positive class (DISEASE = 1), Scores for negative class (DISEASE = 0)
pr_prob_full <- pr.curve(scores.class0 = ALL_full$T1GRS_nonMHC[ALL_full$DISEASE == 2],
                         scores.class1 = ALL_full$T1GRS_nonMHC[ALL_full$DISEASE == 1],
                         curve = TRUE)
auprc_prob_full <- pr_prob_full$auc.integral

pr_grs2_full <- pr.curve(scores.class0 = ALL_full$sum_All5_nonMHC[ALL_full$DISEASE == 2],
                         scores.class1 = ALL_full$sum_All5_nonMHC[ALL_full$DISEASE == 1],
                         curve = TRUE)
auprc_grs2_full <- pr_grs2_full$auc.integral
...
plot_pr_full <- ggplot(pr_df_full, aes(x = Recall, y = Precision, color = Model)) +
  ...
  scale_color_manual(values = c("T1GRS" = "slateblue", "GRS2" = "firebrick"))
print(plot_pr_full)
#ggsave('figs_may/fig3/NPOD_auprc_nonMHC.pdf', plot = plot_pr_full, width = 6, height = 5)
...
for (i in 1:n_bootstraps) {
  ...
  boot_sample <- ALL %>% sample_n(size = nrow(.), replace = TRUE)
  
  # Ensure the bootstrap sample has both cases and controls
  if (length(unique(boot_sample$T1D)) < 2) {
    t1grs_auprc_boot[i] <- NA # Mark as invalid if only one class is present
    grs2_auprc_boot[i] <- NA
    next # Skip to the next iteration
  }
  
  # Calculate AUPRC for T1GRS on the bootstrap sample
  pr_t1grs_boot <- pr.curve(scores.class0 = boot_sample$T1GRS_ALL[boot_sample$T1D == 2],
                            scores.class1 = boot_sample$T1GRS_ALL[boot_sample$T1D == 1],
                            curve = FALSE) # No need to store the curve for bootstrapping
  t1grs_auprc_boot[i] <- pr_t1grs_boot$auc.integral
  
  # Calculate AUPRC for GRS2 on the bootstrap sample
  pr_grs2_boot <- pr.curve(scores.class0 = boot_sample$Total_sum[boot_sample$T1D == 2],
                           scores.class1 = boot_sample$Total_sum[boot_sample$T1D == 1],
                           curve = FALSE)
  grs2_auprc_boot[i] <- pr_grs2_boot$auc.integral
}
...
# --- AUPRC CURVE PLOTTING (using original full dataset) ---

# Calculate PR curves on the original full dataset for plotting
pr_t1grs <- pr.curve(scores.class0 = ALL$T1GRS_ALL[ALL$T1D == 2], scores.class1 = ALL$T1GRS_ALL[ALL$T1D == 1], curve = TRUE)
auprc_t1grs <- pr_t1grs$auc.integral

pr_grs2 <- pr.curve(scores.class0 = ALL$Total_sum[ALL$T1D == 2], scores.class1 = ALL$Total_sum[ALL$T1D == 1], curve = TRUE)
auprc_grs2 <- pr_grs2$auc.integral

# --- MODIFIED: Create formatted text strings with CIs for the plot labels ---
t1grs_label_pr <- paste0("T1GRS AUPRC = ", round(auprc_t1grs, 3), " (", round(t1grs_ci[1], 3), "-", round(t1grs_ci[2], 3), ")")
grs2_label_pr <- paste0("GRS2 AUPRC = ", round(auprc_grs2, 3), " (", round(grs2_ci[1], 3), "-", round(grs2_ci[2], 3), ")")
...
# Create data frames for ggplot for PR curves
pr_t1grs_df <- data.frame(Recall = pr_t1grs$curve[, 1], Precision = pr_t1grs$curve[, 2], Model = "T1GRS")
pr_grs2_df <- data.frame(Recall = pr_grs2$curve[, 1], Precision = pr_grs2$curve[, 2], Model = "GRS2")
pr_df <- bind_rows(pr_t1grs_df, pr_grs2_df)

# Calculate baseline for PR curve (proportion of positive cases)
pr_baseline <- sum(ALL$T1D == 2) / nrow(ALL)
...
plot_pr <- ggplot(pr_df, aes(x = Recall, y = Precision, color = Model)) +
  ...
  labs(title = pr_plot_title,
       ...
       color = "Model") +
  ylim(0, 1) +
  # --- MODIFIED: Annotate with AUPRC values and CIs ---
  annotate("text", x = 0.65, y = 0.25, label = t1grs_label_pr, color = "slateblue", size = 4) +
  annotate("text", x = 0.65, y = 0.20, label = grs2_label_pr, color = "firebrick", size = 4) +
  annotate("text", x = 0.65, y = 0.15, label = paste0("Baseline = ", round(pr_baseline, 3)), color = "grey50", size = 4) +
  scale_color_manual(values = c("T1GRS" = "slateblue", "GRS2" = "firebrick"))
...
print(plot_pr)
ggsave('/home/jupyter/workspaces/t1dgrssupplement/tj_folder/AoU_full_auprc_with_CIs.pdf', plot = plot_pr, width = 6, height = 5)
